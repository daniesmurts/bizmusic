...
const SYSTEM_PROMPT = `Ты профессиональный копирайтер для российского бизнеса. Твоя задача — создавать лаконичные, привлекательные и профессиональные объявления, подходящие для торговых залов, кафе, ресторанов и бутиков.
...
Ответ должен содержать ТОЛЬКО текст объявления, без пояснений.`;
...
export async function refineAnnouncementText(
  ...
  const messages: GroqChatMessage[] = [
    ...
    {
      role: "user",
      content: `Улучши это объявление для бизнес-аудитории:\n\n"${userDraft}"`,
    },
  ];

...
export default function ContractPage() {
  ...
  const addTradePoint = () => setTradePoints([...tradePoints, ""]);
      ...
      <div className="glass-dark border border-white/10 rounded-[3rem] p-10 space-y-12 shadow-2xl shadow-neon/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <select 
                ...
                className="w-full bg-neutral-900/50 border border-white/10 rounded-2xl p-4 text-white focus:border-neon outline-none transition-all appearance-none h-14"
              >
            ...
            <div className="space-y-4">
              ...
              <Input 
                ...
                className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14 focus:border-neon/50" 
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-4">
                ...
                <Input 
                  ...
                  placeholder="12 цифр" 
                  className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" 
                />
              </div>
              <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <LocationInput value={regAddress} onChange={setRegAddress} placeholder="Юридический адрес" />
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <Input value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="Название банка" className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" />
            </div>

            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-4">
                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 px-1">БИК</Label>
                <Input value={bik} onChange={(e) => setBik(e.target.value)} placeholder="9 цифр" className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" />
              </div>
              <div className="space-y-4">
                <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 px-1">Телефон</Label>
                <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+7 (___) ___-__-__" className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" />
              </div>
            </div>

            <div className="space-y-4">
              <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 px-1">Расчетный счет</Label>
              <Input value={settlementAccount} onChange={(e) => setSettlementAccount(e.target.value)} placeholder="20 цифр" className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" />
            </div>

            <div className="space-y-4">
              <Label className="text-[10px] font-black uppercase tracking-widest text-neutral-500 px-1">Корр. счет</Label>
              <Input value={corrAccount} onChange={(e) => setCorrAccount(e.target.value)} placeholder="20 цифр" className="bg-neutral-900/50 border-white/10 rounded-2xl p-6 text-white h-14" />
            </div>
          </div>
        </div>
        ...
        <div className="pt-8 space-y-6">
           <div className="flex items-center justify-between border-b border-white/5 pb-4">
             ...
             <Button onClick={addTradePoint} variant="ghost" className="text-neon hover:text-white transition-colors uppercase font-black text-[10px] tracking-widest gap-2">
             ...
             {tradePoints.map((point, i) => (
                <div key={i} className="flex gap-4 items-center animate-fade-in">
                   <div className="flex-1">
                     <LocationInput value={point} onChange={(val) => updateTradePoint(i, val)} placeholder={`Адрес точки ${i + 1}`} />
                   </div>
                   {tradePoints.length > 1 && (
                     <Button onClick={() => removeTradePoint(i)} variant="ghost" size="icon" className="text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded-xl">
           ...
           <div className="max-w-md mx-auto space-y-6 text-center">
              <div className="space-y-4">
                ...
                <Input 
                  ...
                  className="bg-neutral-900/80 border-2 border-neon/30 focus:border-neon rounded-2xl p-8 text-center text-2xl font-black italic uppercase tracking-wider text-neon shadow-2xl shadow-neon/10" 
                />
              ...
              <Button 
                ...
                disabled={loading}
                className="w-full bg-neon text-black hover:scale-105 transition-all rounded-2xl py-10 font-black uppercase text-sm tracking-widest gap-3 shadow-2xl shadow-neon/30 border-none group"
              >
                {loading ? (
           ...
           {success ? (
             <div className="glass-dark border-2 border-neon/30 p-8 rounded-[2rem] space-y-6 group bg-neon/5 animate-fade-in shadow-2xl shadow-neon/10">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-neutral-500 text-[10px] font-black uppercase tracking-widest">Договор подписан: {new Date().toLocaleDateString("ru-RU")}</p>
                </div>
                <Button 
                  onClick={handleGenerateLicense} // Re-download
                  variant="ghost" 

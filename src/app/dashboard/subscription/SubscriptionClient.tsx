...
interface BusinessData {
  ...
  ttsMonthlyPeriodStart: string | null;
  ttsMonthlyPeriodEnd: string | null;
...
export default function SubscriptionClient({ prices }: { prices: SubscriptionPrices }) {
  ...
  const PLANS_UI = [
    {
      name: "Бизнес",
      slug: "business",
      price: formatRub(isYearly ? Math.round(prices.businessMonthly * 12 * 0.8) : prices.businessMonthly),
      priceValue: isYearly ? Math.round(prices.businessMonthly * 12 * 0.8) : prices.businessMonthly,
    ...
    },
    {
      name: "Контент",
      slug: "content",
      price: formatRub(isYearly ? Math.round(prices.contentMonthly * 12 * 0.8) : prices.contentMonthly),
      priceValue: isYearly ? Math.round(prices.contentMonthly * 12 * 0.8) : prices.contentMonthly,
    ...
    },
    {
      name: "Бизнес Про",
      slug: "business-pro",
      price: formatRub(isYearly ? Math.round(prices.businessProMonthly * 12 * 0.8) : prices.businessProMonthly),
      priceValue: isYearly ? Math.round(prices.businessProMonthly * 12 * 0.8) : prices.businessProMonthly,
    ...
    },
    {
      name: "Бизнес +",
      slug: "business-plus",
      price: formatRub(isYearly ? Math.round(prices.businessPlusMonthly * 12 * 0.8) : prices.businessPlusMonthly),
      priceValue: isYearly ? Math.round(prices.businessPlusMonthly * 12 * 0.8) : prices.businessPlusMonthly,
    ...
    <div className="space-y-8 lg:space-y-12 pb-20 relative z-0 min-h-screen">
      ...
      {businessData?.subscriptionStatus === 'ACTIVE' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="glass-dark border border-white/5 rounded-2xl p-6 relative overflow-hidden">
             <div className="absolute top-0 right-0 p-12 bg-neon/5 blur-[50px] rounded-full -mr-8 -mt-8 pointer-events-none" />
             <div className="flex items-center justify-between mb-4 relative z-10">
               <div className="space-y-1">
                 <p className="text-[10px] font-black uppercase tracking-widest text-neutral-500">Генерации объявлений (TTS)</p>
                 <h4 className="text-xl font-black uppercase">Использовано: {businessData.ttsMonthlyUsed}</h4>
               </div>
             ...
             <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden mb-2">
                <div 
                  ...
                  style={{ width: `${Math.min(100, (businessData.ttsMonthlyUsed / (PLANS_UI.find(p => p.slug === businessData.currentPlanSlug)?.ttsMonthlyLimit || 1)) * 100)}%` }} 
                />
             </div>
             <div className="flex justify-between text-[8px] font-bold uppercase tracking-widest text-neutral-500">
                <span>Лимит: {PLANS_UI.find(p => p.slug === businessData.currentPlanSlug)?.ttsMonthlyLimit || 0}</span>
                {businessData.ttsMonthlyPeriodEnd && (
                  <span>Обновление: {new Date(businessData.ttsMonthlyPeriodEnd).toLocaleDateString('ru-RU')}</span>
                )}
             </div>
          </div>

          <div className="glass-dark border border-white/5 rounded-2xl p-6 relative overflow-hidden">
             <div className="absolute top-0 right-0 p-12 bg-purple-500/5 blur-[50px] rounded-full -mr-8 -mt-8 pointer-events-none" />
             <div className="flex items-center justify-between mb-4 relative z-10">
               <div className="space-y-1">
                 <p className="text-[10px] font-black uppercase tracking-widest text-neutral-500">AI Помощник</p>
                 <h4 className="text-xl font-black uppercase">Использовано: {businessData.aiMonthlyUsed}</h4>
               </div>
             ...
             <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden mb-2">
                <div 
                  ...
                  style={{ width: `${Math.min(100, (businessData.aiMonthlyUsed / (businessData.currentPlanSlug === 'business-plus' ? 10 : 5)) * 100)}%` }} 
                />
             </div>
             <div className="flex justify-between text-[8px] font-bold uppercase tracking-widest text-neutral-500">
                <span>Лимит: {businessData.currentPlanSlug === 'business-plus' ? 10 : 5}</span>
                {businessData.aiMonthlyPeriodEnd && (
                  <span>Обновление: {new Date(businessData.aiMonthlyPeriodEnd).toLocaleDateString('ru-RU')}</span>
                )}
             </div>
          </div>
        </div>
      ...
      {/* Plans Section */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 max-w-7xl mx-auto w-full">
        {PLANS_UI.map((tier) => {
          ...
          return (
            <div key={tier.slug} className={cn(
              ...
              "flex-row items-center gap-4 p-4 sm:flex-col sm:items-stretch sm:p-6 sm:h-full",
              tier.highlight ? "border-neon/30 bg-neon/5 shadow-[0_0_40px_rgba(92,243,135,0.1)]" : tier.popular ? "border-neon/20" : "border-white/5 hover:border-white/10",
              ...
              <div className="flex-1 min-w-0 sm:text-center sm:mb-6 sm:space-y-3">
                <div className="flex items-center gap-2 sm:flex-col sm:gap-0">
                  ...
                  {(tier.popular || tier.highlight) && <span className="text-[8px] font-black uppercase tracking-widest bg-neon/20 text-neon px-2 py-0.5 rounded-full border border-neon/30 sm:mt-1">{tier.highlight ? "Рекомендуем" : "Популярный"}</span>}
                </div>
